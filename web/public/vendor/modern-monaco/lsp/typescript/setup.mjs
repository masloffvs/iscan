...
async function createWorker(monaco, workspace, languageSettings, formattingOptions) {
  ...
  const defaultImportMap = languageSettings?.importMap ?? {};
  ...
  if (fs) {
    ...
    const watchTypeFiles = () => (compilerOptions.$types ?? []).filter((url) => !url.startsWith("https://") && !url.startsWith("http://")).map(
...
function isSameImportMap(im1, im2) {
...
function isSameStringMap(obj1, obj2) {
